...
class Captcha {
  ...
  verifyToken(key) {
    if (process.env.captchaBypass) {
      if (key == process.env.captchaBypass) {
        return true;
      }
    }
    return new Promise(function(resolve, reject) {
      request(`https://www.google.com/recaptcha/api/siteverify?secret=${process.env.captchaKey}&response=${key}`, function(error, response, body) {
        if (error) {
          resolve(false)
          return;
        };
