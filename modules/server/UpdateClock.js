    ...
    updateClock() {
        ...
        for (var i in this.updates) {
            ...
            for (var u = 0; u < pxupdates.length; u++) {
                ...
                /*
                let clientList = server.worlds.find(function(world) {
                    return world.name == i;
                }.bind(this)).clients;
                for(let clid in clientList){
                    if(parseInt(clientList[clid].x_pos) == parseInt(client.x) && parseInt(clientList[clid].y_pos) == parseInt(client.y)) { // will never happen
                        upd_dv.setInt32(offs, clientList[clid].id, true);
                    };
                };
                */
