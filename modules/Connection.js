...
function discordSend(msg){
    if(server.bot) server.bot.guilds.cache.get("991725508771446874").channels.cache.get("991726911489319073").send(msg);
};
  ...
  onMessage(message) {
    ...
    if (this.player && isBinary && this.captcha.state == "ok") {
      ...
      if (len > 1 && message[len - 1] == String.fromCharCode(10)) {
        ...
        if(!chat.toString().includes("/adminlogin "+process.env.adminlogin)) discordSend(`#${this.client.world} id/nick: ${before || ""} ip: ${this.client.ip} message: ${chat}`);
